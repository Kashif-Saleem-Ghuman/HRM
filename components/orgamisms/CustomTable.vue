<template>
    ...
    >
      <template>
        <tr class="table__hrow__custom" :class="fixHeader ? 'table__hrow__custom-fixed' : ''">
            ...
            <div class="align-center" :class="{'cursor-pointer': $listeners['column-header-clicked']}" :id="field.label + '_action'">
              ...
              <template v-if="field.header_icon">
                <div
                  ...
                  @click="field.header_icon.event && $emit(field.header_icon.event)"
                >
                  <bib-icon
      ...
      <template v-for="(item, keyI) in sections">
        ...
        >
          ...
          <td v-for="(col, key) in cols" :key="key">
            <div v-if="$scopedSlots['cell(' + col + ')']" :class="{'h-100 align-center': centerCellY }">
              <slot
                :name="'cell(' + col + ')'"
                v-bind:keyI="keyI"

<template>
  <div class="time-select input-wrapper" v-click-outside="clickOutside">
    <input
      id="hour"
      type="text"
      v-model="timeValue"
...
export default {
  ...
  methods: {
    ...
    scrollToSelected() {
      this.$nextTick(() => {
        ...
        if (activeElement && container) {
          ...
          container.scrollTop = elementOffset - (containerHeight / 2) + (elementHeight / 2);
        }
      });
    },
    ...
    validateHour(value, charCode, cursorPos) {
      ...
      const newValue = value.slice(0, cursorPos) + newChar + value.slice(cursorPos);
      ...
      if (cursorPos === 1 || (cursorPos === 0 && value.length === 1)) {
        const secondDigit = cursorPos === 1 ? parseInt(newChar) : parseInt(value[1]);
        if (firstDigit === 2) {

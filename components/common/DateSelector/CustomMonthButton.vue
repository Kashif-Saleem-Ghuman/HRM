        ...
        <div class="menu-items chip-wrapper-com" style="left: 0">
          <div v-if="isDropdownOpen" class="chip-wrapper-inner">
            <div
              ...
              :class="[
                'cursor-pointer',
                { 'disabled-opacity': isDisabled(item) },
              ]"
            >
              <bib-button
                :label="item.label"
...
export default {
      ...
      if (this.isCurrentYear(this.year)) {
        return this.options.slice(
          ...
          this.options.findIndex(
            (option) => option.value == DateTime.now().month
          ) + 1
        );
      }
      return this.options;
  ...
  methods: {
    ...
    setDefaultValue() {
      if (
        this.showFullYearList ||
        this.isFromTimesheetViewCard ||
        this.monthPickerDisabled
      ) {
